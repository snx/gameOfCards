...
/***/ function(module, exports, __webpack_require__) {
	...
	var TweenState = __webpack_require__(159);

	var ScoresCounter = React.createClass({
	  displayName: 'ScoresCounter',

	  getInitialState: function () {
	    return { scoresCount: 0 };
	  },

	  updateScores: function (updateValue) {
	    this.setState({ scoresCount: this.state.scoresCount + updateValue });
	  },

	  componentWillMount: function () {
	    this.props.onRegUpdateScoresCallback(this.updateScores);
	  },

	  render: function () {
	    var scoresCountStr = Math.abs(this.state.scoresCount).toString();
	    var threeDigitsScores = this.state.scoresCount < 0 ? '-' : '';
	    for (var i = 0; i < 3 - scoresCountStr.length; i++) {
	      threeDigitsScores += '0';
	    }
	    threeDigitsScores += scoresCountStr;

	    var scoresColor = '';
	    if (this.state.scoresCount < 30) {
	      scoresColor = 'pink';
	    } else if (this.state.scoresCount < 80) {
	      scoresColor = 'navajowhite';
	    } else {
	      scoresColor = 'lightgreen';
	    }
	    ...
	    return React.createElement(
	      'div',
	      { className: 'scoresCounter', style: colorStyle },
	      threeDigitsScores
	    );
	  }
	});
	...
	var Card = React.createClass({
	  ...
	  getInitialState: function () {
	    var initialStyle = {
	      margin: 26 + Math.random() * 5 + 'px',
	      top: Math.random() * 30 + 'px',
	      right: Math.random() * 30 + 'px',
	      transform: 'rotate(' + Math.random() * 100 + 'deg)'
	    };

	    return { style: initialStyle, id: this.props.id, pairId: this.props.pairId, isFlipped: this.props.isFlipped, isHidden: false };
	  },

	  componentWillReceiveProps: function (nextProps) {},

	  clickHandler: function (e) {
	    this.props.onFlip(this.state.id, this.state.pairId, this.flip, this.successfulFlip, this.unsuccessfulFlip);
	  },

	  successfulFlip: function () {
	    if (this.state.isFlipped) {
	      ...
	      this.flip(hideAction, 0);
	    }
	  },

	  unsuccessfulFlip: function (newPairId) {
	    var context = this;
	    if (this.state.isFlipped) {
	      var flipAction = function () {
	        context.flip(null, 1000);
	      };
	      this.swap(newPairId, flipAction, 500);
	    } else {
	      var swapAndFlip = function () {
	        var flipAction = function () {
	          context.flip(null, 1000);
	        };
	        context.swap(newPairId, flipAction, 500);
	      };

	      this.flip(swapAndFlip, 0);
	    }
	  },

	  flip: function (endCallback, actionDelay) {
	    if (endCallback) {
	      this.tweenState('isFlipped', { delay: actionDelay, endValue: !this.state.isFlipped, onEnd: endCallback });
	    } else {
	      this.tweenState('isFlipped', { delay: actionDelay, endValue: !this.state.isFlipped });
	    }
	  },

	  swap: function (newPairId, endCallback, actionDelay) {
	    if (endCallback) {
	      this.tweenState('pairId', { delay: actionDelay, duration: 0, endValue: newPairId, onEnd: endCallback });
	    } else {
	      this.tweenState('pairId', { delay: actionDelay, duration: 0, endValue: newPairId });
	    }
	  },

	  hide: function (actionDelay) {
	    this.tweenState('isHidden', { delay: actionDelay, endValue: true });
	  },

	  shouldComponentUpdate: function (nextProps, nextState) {
	    return true;
	    // return this.state.id != nextState.id || this.state.isFlipped != nextState.isFlipped || this.state.isHidden != nextState.isHidden;
	  },

	  render: function () {
	    var updatedStyle = JSON.parse(JSON.stringify(this.state.style));
	    updatedStyle.backgroundImage = "url('images/" + (this.getTweeningValue('isFlipped') ? +this.getTweeningValue('pairId') + '.jpg' : 'poo.png') + "')";
	    updatedStyle.visibility = this.getTweeningValue('isHidden') ? 'hidden' : 'visible';

	    return React.createElement('div', { className: 'card', style: updatedStyle, onClick: this.clickHandler });
	  }
	});

	var GocApp = React.createClass({
	  displayName: 'GocApp',
	  ...
	  getInitialState: function () {
	    ...
	    var pairsNumber = 10;
	    for (var i = 0; i < 2 * pairsNumber; i++) {
	      var pairId;
	      do {
	        pairId = Math.floor(Math.random() * 10);
	      } while (usedIds[pairId] == 2);

	      cards.push(this.generateCard({ pairId: pairId, isFlipped: false }, i));
	      usedIds[pairId] = !usedIds[pairId] ? 1 : ++usedIds[pairId];
	    }

	    var scoresCounter = React.createElement(ScoresCounter, { onRegUpdateScoresCallback: this.regUpdateScoresCallback });
	    return { pairsNumber: pairsNumber, cards: cards, cardFlipCounts: new Array(pairsNumber), flippedCardIds: [], pairIds: [], successCallbacks: [], failCallbacks: [], scoresCounter: scoresCounter };
	  },

	  regUpdateScoresCallback: function (callback) {
	    this.setState({ updateScoresCallback: callback });
	  },

	  cardFlipHandler: function (cardId, pairId, flipCallback, successCallback, failCallback) {
	    if (this.state.flippedCardIds.length < 2 && this.state.flippedCardIds.indexOf(cardId) == -1) {
	      this.state.flippedCardIds.push(cardId);
	      this.state.pairIds.push(pairId);
	      this.state.successCallbacks.push(successCallback);
	      this.state.failCallbacks.push(failCallback);

	      if (this.state.flippedCardIds.length == 2) {
	        var isMatch = this.state.pairIds[0] == this.state.pairIds[1];
	        if (isMatch) {
	          this.state.successCallbacks[0]();
	          this.state.successCallbacks[1]();
	          this.state.updateScoresCallback(10);
	        } else {
	          this.state.failCallbacks[0](this.state.pairIds[1]);
	          this.state.failCallbacks[1](this.state.pairIds[0]);

	          if (this.state.cardFlipCounts[cardId] > 0) {
	            this.state.updateScoresCallback(-(this.state.cardFlipCounts[cardId] * 2));
	          }
	        }

	        this.setState({ flippedCardIds: [], pairIds: [], successCallbacks: [], failCallbacks: [] });
	      } else {
	        flipCallback();
	      }

	      this.state.cardFlipCounts[cardId] = !this.state.cardFlipCounts[cardId] ? 1 : ++this.state.cardFlipCounts[cardId];
	    }
	  },
	      ...
	      React.createElement(
	        ...
	        React.createElement(
	          ...
	          React.createElement(
	            'li',
	            null,
	            'Flip cards to find pairs and clear the board.'
	          ),
	          React.createElement(
	            'li',
	            null,
	            'Different cards will be swapped.'
	          ),
	          React.createElement(
	            'li',
	            null,
	            'Remember places of cards.'
	          ),
	          React.createElement(
	            'li',
	            null,
	            'Earn as much scores as you can!'
	          )
	        ),
	        this.state.scoresCounter
	      ),
	      this.state.cards
	    );
	  }
	...
	ReactDOM.render(React.createElement(GocApp, null), document.getElementById('mainDiv'));
